...
pool.post('/add-liquidity', async (c) => {
    try {
        const body = await c.req.json();
        const { poolId, token0, token1, fee, tickSpacing, hookAddress, amount0, amount1, userAddress, tickLower, tickUpper } = body;
        // Validation
        if (!poolId) {
            return c.json({ success: false, message: 'Pool ID is required.' }, 400);
        }
        ...
        const result = await (0, poolService_1.addLiquidity)({
            poolId,
            token0,
            token1,
            fee,
...
pool.get('/balances/:userAddress', async (c) => {
    try {
        ...
        const result = await (0, poolService_1.getTokenBalances)(userAddress, tokens);
        if (!result.success) {
            return c.json(result, 400);
        }
        return c.json(result, 200);
        ...
        throw new http_exception_1.HTTPException(500, { message: 'Internal Server Error while getting token balances.' });
    }
});
// Health check
pool.get('/health', (c) => {
